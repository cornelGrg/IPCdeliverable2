...
std::vector<float> flatten(const std::vector<std::vector<float>>& mat) {
    ...
    for (const auto& row : mat) {
        flat.insert(flat.end(), row.begin(), row.end()); // Append all elements of the row
    }
    return flat;
}


std::vector<std::vector<float>> deflatten(const std::vector<float>& flat) {
    // Calculate the size of the matrix
    int size = static_cast<int>(sqrt(flat.size()));

    // Check if the flattened array is squared
    if (size * size != flat.size()) {
        throw std::invalid_argument("The flattened array length is not a perfect square.");
    }
